"""
BLuxA Corp Transportation Management System - Final Production Backend
Complete with all polish and validation improvements for production launch
"""
...
def check_super_admin_exists():
    ...
    global SUPER_ADMIN_CREATED
    if SUPER_ADMIN_CREATED is None:
        try:
            result = supabase.table('admin_users').select('*').eq('role', 'super_admin').execute()
            SUPER_ADMIN_CREATED = len(result.data) > 0
        except Exception:
...
def get_pricing_from_db(vehicle_type: str) -> Dict:
    """
    Get pricing from DB with proper minimum handling.
    Priority:
...
def notify_all_admins(notification_type: str, title: str, message: str, metadata: dict = None):
    """Create notifications for all admin users"""
    try:
        admins = supabase.table('admin_users').select('id').eq('status', 'active').execute()
        ...
        logger.error(f"Failed to notify all admins: {str(e)}")
...
def retry_failed_notifications():
    """Enhanced background function to retry failed notifications and old pending ones"""
    try:
        ...
        old_pending_notifications = supabase.table('notifications').select('*').eq('status', 'pending').lt('created_at', five_minutes_ago).execute()
        
        all_notifications = failed_notifications.data + old_pending_notifications.data
        ...
        for notification in all_notifications:
            ...
            if notification['type'] in ['booking_confirmation', 'payment_confirmation', 'driver_assigned', 'status_update', 'welcome']:
                if notification.get('email_sent') is False or notification.get('email_sent') is None:
                    # Retry email
...
def health_check():
    """Enhanced health check endpoint with service verification"""
    try:
...
def api_docs():
    ...
    try:
        # Load OpenAPI spec
        openapi_file = os.path.join(os.path.dirname(__file__), 'bluxa_openapi.yaml')
        if os.path.exists(openapi_file):
            ...
            spec = {
                "openapi": "3.0.3",
                "info": {
                    "title": "BLuxA Corp Transportation API",
                    "version": "1.0.0",
                    "description": "Luxury transportation management system API"
                },
        ...
        spec['servers'] = [{"url": request.host_url.rstrip('/'), "description": "Current server"}]
        ...
        <html>
        <head>
            <title>BLuxA Corp API Documentation</title>
        ...
        return jsonify({'error': f'Failed to load API documentation: {str(e)}'}), 500
        ...
        if auth_response.user:
            ...
            if role == 'driver':
                driver_data = {
                    ...
                    'total_rides': 0  # Default total rides
                }
...
def update_booking_status(booking_id):
    ...
    try:
        ...
        if result.data:
            ...
            # Update driver total_rides if booking completed
            if new_status == 'completed' and booking.get('driver_id'):
                supabase.table('drivers').update({
                    'total_rides': supabase.table('drivers').select('total_rides').eq('id', booking['driver_id']).execute().data[0]['total_rides'] + 1
                }).eq('id', booking['driver_id']).execute()
...
# Enhanced Admin endpoints with improved assignment logic
...
def admin_create_vehicle():
    ...
    vehicle_payload = {
        'vehicle_id': data.get('vehicle_id') or generate_vehicle_id(),
        'make': data['make'],
        'model': data['model'],
        'year': int(data['year']),
...
def admin_create_driver():
    ...
    driver_payload = {
        'driver_id': data.get('driver_id') or generate_driver_id(),
        'email': data['email'],
        ...
        'rating': float(data.get('rating', 0.0)),  # Default rating
        'total_rides': int(data.get('total_rides', 0)),  # Default total rides
    }
...
def admin_update_driver(driver_uuid):
    ...
    allowed_fields = {
        ...
        'phone', 'first_name', 'last_name', 'rating', 'total_rides'
    }
    update_payload = {k: data[k] for k in data.keys() if k in allowed_fields}
    if not update_payload:
        return jsonify({'error': 'No allowed fields to update'}), 400
...
# Enhanced Vehicle assignment endpoints with improved status logic
...
def create_assignment():
    """Create vehicle assignment with improved status logic"""
    data = request.get_json() or {}
    ...
    # IMPROVED: Mark driver and vehicle as "assigned" (not "available")
    supabase.table('drivers').update({'status': 'assigned'}).eq('id', data['driver_id']).execute()
    supabase.table('vehicles').update({'status': 'assigned'}).eq('id', data['vehicle_id']).execute()
...
def close_assignment(assignment_uuid):
    """Close vehicle assignment with improved status logic"""
    data = request.get_json() or {}
    unassigned_date = data.get('unassigned_date') or datetime.utcnow().date().isoformat()

    # Get assignment details first
    assignment_result = supabase.table('vehicle_assignments').select('*').eq('id', assignment_uuid).execute()
    ...
    if not r.data:
        return jsonify({'error': 'Assignment update failed'}), 404

    # IMPROVED: Revert driver and vehicle status to "available"
    supabase.table('drivers').update({'status': 'available'}).eq('id', assignment['driver_id']).execute()
    supabase.table('vehicles').update({'status': 'available'}).eq('id', assignment['vehicle_id']).execute()
...
@auth_required(['admin'])
def admin_settings():
    """Get or update admin settings with enhanced notifications"""
    try:
        if request.method == 'GET':
            ...
            # ENHANCED: Notify all admin users about settings change
            notify_all_admins(
                'settings_updated',
                'System Settings Updated',
                f'Setting "{setting_key}" has been updated to "{setting_value}" by {request.user.user_metadata.get("full_name", "Admin")}.',
                {'setting_key': setting_key, 'setting_value': setting_value, 'updated_by': request.user.id}
            )
...
# Notification retry endpoint
@app.route('/admin/notifications/retry', methods=['POST'])
@auth_required(['admin'])
@limiter.limit("10 per hour")
def trigger_notification_retry():
    """Triggers retry of failed notifications and old pending ones"""
...
# Go-Live Checklist endpoint
@app.route('/admin/checklist', methods=['GET'])
@auth_required(['admin'])
def go_live_checklist():
    """Production readiness checklist for go-live validation"""
    try:
        checklist = {}
        ...
        try:
            ...
            checklist['health'] = 'ok' if health_response[1] == 200 else 'error'
        except Exception:
            checklist['health'] = 'error'
        
        # Stripe webhook verification
        try:
            # Check if we have webhook secret configured
            checklist['stripe_webhook_verified'] = bool(STRIPE_WEBHOOK_SECRET)
        except Exception:
            checklist['stripe_webhook_verified'] = False
        
        # Supabase connection
        try:
            supabase.table('users').select('id').limit(1).execute()
            checklist['supabase_connection'] = 'ok'
        except Exception:
            checklist['supabase_connection'] = 'error'
        
        # Pending notifications count
        try:
            pending_notifications = supabase.table('notifications').select('id', count='exact').eq('status', 'pending').execute()
            checklist['pending_notifications'] = pending_notifications.count
        except Exception:
            checklist['pending_notifications'] = 0
        
        # Failed notifications count
        try:
            failed_notifications = supabase.table('notifications').select('id', count='exact').eq('status', 'failed').execute()
            checklist['failed_notifications'] = failed_notifications.count
        except Exception:
            checklist['failed_notifications'] = 0
        
        # Super admin exists
        try:
            super_admin_exists = check_super_admin_exists()
            checklist['super_admin_exists'] = super_admin_exists
        except Exception:
            checklist['super_admin_exists'] = False
        ...
        critical_checks = [
            checklist['health'] == 'ok',
            checklist['supabase_connection'] == 'ok',
            checklist['super_admin_exists'],
            checklist['environment_variables']['supabase_configured'],
            checklist['environment_variables']['stripe_configured']
        ]
        
        checklist['readiness_score'] = sum(critical_checks) / len(critical_checks) * 100
        checklist['production_ready'] = checklist['readiness_score'] >= 80
        
        return jsonify(checklist), 200
        
    except Exception as e:
        logger.error(f"Go-live checklist error: {str(e)}")
...
def driver_dashboard():
    """Enhanced driver dashboard with rating and total_rides from DB"""
    try:
        user_id = request.user.id
        
        # Get driver details with rating and total_rides from DB
        ...
        return jsonify({
            ...
            'stats': {
                ...
                'total_rides': driver['total_rides'],  # From DB
                'week_earnings': total_earnings,
                'rating': driver['rating']  # From DB
            },
...
# Enhanced Seed super admin endpoint with security
@app.route('/seed/super-admin', methods=['POST'])
def seed_super_admin():
    """Create initial super admin account with enhanced security"""
    try:
        ...
        # ENHANCED: Check if super admin already exists and disable if so
        if check_super_admin_exists():
            return jsonify({
                'message': 'Super admin already exists',
                'note': 'Seed endpoint is disabled for security'
            }), 200
        ...
        if auth_response.user:
            ...
            return jsonify({
                ...
                'email': admin_email,
                'password': admin_password,
                'note': 'Seed endpoint is now disabled for security'
            }), 201
        else:
...
# Enhanced background task to periodically retry failed notifications
def start_notification_retry_scheduler():
    ...
    retry_thread.start()
    logger.info("Enhanced notification retry scheduler started")
